...
export default class Purchase_Details_Screen extends Component {
    ...
    AsyncStorage.getItem('CurrentCompanyName', (err, result) => {
      ...
      if (result !== null) {
        ...
        db.transaction(tx => {
          ...
          tx.executeSql('SELECT * FROM  Purchase where IsActive =1 AND PurchaseID = ? ',
            [this.state.PurchaseID], (tx, results) => {
          ...
          tx.executeSql('SELECT * FROM  PurchaseDetails where IsActive =1 AND PurchaseID = ? ',
            [this.state.PurchaseID], (tx, results) => {
              var temp = [];
  ...
  componentDidMount() {
    this.backHandler = BackHandler.addEventListener("hardwareBackPress", () => {
        ...
        this.props.navigation.navigate('ALL_PURCHASE_SCREEN',{transType:this.state.onBackTransactionType});
    ...
    AsyncStorage.getItem('CurrentCompanyName', (err, result) => {
      ...
      if (result !== null) {
        ...
        db.transaction(tx => {
          ...
          tx.executeSql('UPDATE Purchase  SET  OnCash = 1 where  PurchaseID = ? ',
          [this.state.PurchaseID], (tx, results) => {

            if (results.rowsAffected > 0) {
              Alert.alert(
                'Success',
                'Purchase Clear Successfully',
                [
                  {
                    text: 'Ok',
                    onPress: () =>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.upperView}>
          ...
          <View style={{ flexDirection: "row", justifyContent: "flex-start", alignContent: "center",marginTop:12 }}>
            ...
            <Text style={{ fontSize: 24, color: colors.textOnDark }}>{this.state.InvoiceDetails.SupplierName}</Text>
          </View>
        ...
        <FlatList style={styles.flatelistStyle}
          ...
          renderItem={({ item }) => (
            <View key={item.PurchaseID} style={styles.flatelistViewStyle}>
              <TouchableOpacity style={styles.buttonStyle}>
                <View style={{ flexDirection: "row" }}>
                  <View style={{ width: "45%", justifyContent: "center", backgroundColor: colors.primary }}>
                    ...
                    <Text style={{ fontSize: 18, color: colors.onPrimary, marginLeft: '10%' }}>Amount :  {item.PerUnitCostprice * item.ItemQuantity}</Text>
                  </View>
                  <View style={{ flexDirection: "column", justifyContent: "center", alignItems: "flex-start", backgroundColor: colors.primaryLight, width: '55%' }}>
                    <Text style={{ fontSize: 16, color: colors.textOnDark,marginLeft:20 }}>Unit Price  :  {item.PerUnitCostprice}</Text>
